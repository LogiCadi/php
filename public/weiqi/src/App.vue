...
export default {
  globalData: {
          ...
          if (res.code) {
            ...
            uni.getUserInfo({
              ...
              success: function (res2) {
                ...
                that.process_WXAccount(code, encryptedData, iv, username, img, gender, callback);
              },
              fail: function () {
                uni.showModal({
                  title: '警告',
    ...
    process_WXAccount(code, encryptedData, iv, username, img, gender, callback) {
      let url = "https://www.gog361.com/flask/v1/joygo/weixin/code2session3/" + gender;
      var that = this;
      uni.request({
        url: url,
        data: {
    ...
    isLegalUser: function () {
      if (this.userid > 0 && this.userInfo != null && this.sessionkey != "" && this.sessionkey.length > 0) {
        return true;
      }
    ...
    getProductList() {
      if (this.userInfo == null || this.sessionkey == null || this.sessionkey == "") {
        return;
      }

      var that = this;
      let theurl = "https://www.gog361.com/flask/v1/joygo/products/" + this.userid;
      uni.request({
        url: theurl,
        data: {
    ...
    toShowBuyProduct() {
      ...
      if (this.productlist != null) {
        ...
        for (let i = 0; i < 5; i++) {
          let text = this.productlist[i].name + ": 价格" + this.productlist[i].price / 10 + "RMB";
          items.push(text);
        }
    ...
    renewSessionKey: function (callback) {
      ...
      let theurl = "https://www.gog361.com/flask/v1/joygo/weixin/renew/" + this.userid;
      uni.request({
        url: theurl,
        header: {
          'content-type': 'application/json'
    ...
    toBuyProduct(productid) {
      ...
      if (res.platform == 'ios') {
        uni.showModal({
          title: '提示',
          content: '由于政策限制，苹果手机不可以购买，请使用安卓手机或者其他途径购买,也可联系微信wyz785购买。'
        });
        return;
      }

      var that = this;
      let theurl = "https://www.gog361.com/flask/v1/joygo/order/xiaochengxu/" + this.userid + "/" + productid + "/0";
      uni.request({
        url: theurl,
        data: {
          sessionkey: this.sessionkey,
